...
* **Preservación del Nombre al Renombrar el Slot Seleccionado:** Al destacar el slot seleccionado (dibujado con fondo coloreado), se limpia su propiedad `inp.label = " "` para evitar que LiteGraph/ComfyUI duplique el texto renderizado en pantalla (evitando el texto gris superpuesto).
  - *Problema:* El diálogo **Rename Slot** de ComfyUI (frontend ≥1.51) hace `input.value = getInputInfo(slot).label || ""`. No usa `getContextMenuOptions`. El slot activo tiene `label = " "` (espacio), así que el popup sale en blanco. Confirmar OK con ese espacio también borraba el nombre custom.
  - *Solución:* Seguir ocultando el label nativo con `" "` en draw. Al leer el slot, `getInputInfo` restaura el título real desde `customLabels` / `_311label` / `inp.name` justo antes de que el diálogo copie el valor. `getContextMenuOptions` hace lo mismo por si algún menú aún lo llama. El siguiente frame de draw vuelve a poner `" "` en el slot seleccionado.
