...
* **Preservación del Nombre al Renombrar el Slot Seleccionado:** Al destacar el slot seleccionado (dibujado con fondo coloreado), se limpiaba su propiedad `inp.label = " "` para evitar que LiteGraph duplicara el texto renderizado en pantalla.
  - *Problema:* Al intentar renombrar el slot activo, la caja de texto del diálogo aparecía en blanco (con un único espacio `" "`). Además, ComfyUI dibuja las etiquetas de los slots después de `onDrawForeground` en algunas configuraciones, lo que volvía a superponer el texto en gris (efecto de doble texto).
  - *Solución:* Dividir la gestión de la etiqueta en dos fases de renderizado: guardar y limpiar la etiqueta en el hook `onDrawBackground` (ejecutado antes del render de LiteGraph) y programar su restauración inmediata al finalizar la pila de ejecución del renderizado actual mediante `setTimeout(..., 0)`. Esto mantiene `inp.label` en `" "` durante toda la fase de renderizado de slots (tanto en LiteGraph como en ComfyUI), pero restaura el valor real justo después de la ejecución del frame actual, haciéndola disponible para las interacciones del menú contextual (renombrar/serializar).
